...
| **Semgrep MCP** | official Semgrep | ~680 | **Install (security).** Conversational SAST: scan a diff for injection, unsafe deserialization and secret patterns while the code is being written, not three days later in CI. The `semgrep` CLI in CI stays regardless — this is the interactive half. |
